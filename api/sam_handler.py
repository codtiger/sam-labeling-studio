...
async def embed_image(
    image_file: UploadFile = File(..., description="Image file to embed.")
):
    """
...
async def predict_on_image(
    ...
    if not getattr(
        predictor, "image_set", True
    ):  # Check if 'image_set' attribute exists and is True
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        np_boxes = [
            (np.array(b, dtype=np.float32) if b else None) for b in request_data.boxes
        ]
        ...
        for points, box in zip(np_point_groups, np_boxes):
            ...
            logger.debug(
                f"Predicting with points: {points is not None}, box: {box is not None}"
            )
        ...
        logger.error(
            f"Error during prediction for image_id {image_id}: {e}", exc_info=True
        )
